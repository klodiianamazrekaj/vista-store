            ...
            for (const item in artikujtNeKarroce[items]) {
                if (artikujtNeKarroce[items][item] > 0) {
                    teDhenaTePerkohshme.push({
                        _id: items,
                        size: item,
                        quantity: artikujtNeKarroce[items][item]
                    })
                }
            }
        }
                        ...
                        return (
                            <div key={index} className="py-4 border-t border-b text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4">
                                <div className="flex items-start gap-6">
                                    <img src={teDhenatEProdukteve.image[0]} className="w-16 sm:w-20" alt="" />
                                    <div>
                                        <p className="text-sm sm:text-lg font-medium">{teDhenatEProdukteve.name}</p>
                                        <div className="flex items-center gap-5 mt-2">
                                            <p>{valuta}{teDhenatEProdukteve.price}</p>
                                            <p className="px-2 sm:px-3 sm:py-1 border bg-slate-50">{item.size}</p>
                                        </div>
                                    </div>
                                </div>
                                <input onChange={(e) => e.target.value === '' || e.target.value === '0' ? null : perditesoTotalinProdukteve(item._id, item.size, Number(e.target.value))} className="border max-w-10 sm:max-w-20 px-1 sm:px-2 py-1" type="number" min={1} defaultValue={item.quantity} />
                                <img onClick={() => perditesoTotalinProdukteve(item._id, item.size, 0)} src={assets.bin_icon} className="w-4 mr-4 sm:w-5 cursor-pointer" alt="" />
                            </div>
                        )
                    })
                }
